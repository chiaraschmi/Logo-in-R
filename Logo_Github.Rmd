...
Function, that stops the functions from plotting (for loops)

```{r}
Dontplot <- function(turtle_state) {
  turtle_state$plot_turtle <- 0
}

Dontplot(turtle_state)

```

Function, that makes the functions plot again
```{r}
doplot <-function(turtle_state) {
...
draw_sphere <- function() {
  ...
  for (i in 1:20) {
    for (j in 1:180) {
      FD(1)  #use FD no plot, so it doens't redraw the plot for every step
      RT(2)  #same here
    }
